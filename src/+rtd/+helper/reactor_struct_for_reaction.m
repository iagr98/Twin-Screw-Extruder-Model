function reactor_test = reactor_struct_for_reaction(e, c_in)
    
    reactor_test = [];
    reactor_test(1).Volume = e.reactorConfiguration{1}.V;
    % reactor_test(1).InitialConcentration = [7.7277 0.015 0.003 0 0 0 0];
    % reactor_test(1).InitialConcentration = c_in(1:7);
    ...
    for i = 2:length(e.reactorConfiguration)-1
        ...
        % reactor_test(i).InitialConcentration = [7.7277 0.015 0.003 0 0 0 0];
        % reactor_test(i).InitialConcentration = c_in((i-1)*7+1:(i-1)*7+7);
    ...
    % reactor_test(i+1).InitialConcentration = [7.7277 0.015 0.003 0 0 0 0];
    % reactor_test(i+1).InitialConcentration = c_in((i-1)*7+1:(i-1)*7+7);
